...
@Component
public class AdminTransactionValidator implements Validator {
	...
	public void validate(Object target, Errors errors) {
		...
		if (trx.getTransactionAmount() <= 0) {
			errors.rejectValue("transactionAmount", "transaction.transactionAmount.empty", "Amount must be greater than 0.");
		}
		...
		case WITHDRAW:
			...
			if (trx.getFromAccountId() != null && !acctService.existsById(trx.getFromAccountId() ) ) {
				errors.rejectValue("fromAccountId", "transaction.fromAccountId.empty", "Account Id does not exist.");
			}
